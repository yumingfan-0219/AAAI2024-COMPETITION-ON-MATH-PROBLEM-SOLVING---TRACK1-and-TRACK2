...
def main():
    ...
    output = []
    count = 0
    with open('/work/cache/paper_zhuanli_game/aaai2024comp/result/TAL_SAQ7K_CN_prediction_72borg.json','r',encoding='utf8') as file:
        data = json.load(file)
        with open('/work/cache/paper_zhuanli_game/aaai2024comp/TAL-SAQ7K-CN.jsonl','r',encoding='utf8') as file_que:
            for line in tqdm(file_que):
                count += 1
                q_id, question = json.loads(line)['queId'], json.loads(line)['problem']
                thinking = data[q_id]
                pre_prompt = '问题：'+question+'\n解题过程：'+thinking+'\n根据上述问题和解题过程，问题的最终答案是多少？请以浮点数的形式直接返回答案，请只返回一个浮点数。'
                text = question
                messages = pre_prompt + text #+ end_prompt
                res, _ = model.chat(tokenizer, messages, history=[])
                if count % 10 ==0:
                    print(question, thinking, res)
                #res = parse_string(res)
                data[q_id] = res

    with open('/work/cache/paper_zhuanli_game/aaai2024comp/math_dataset/school_math_0.25M_summ.json', 'w', encoding='utf8') as file:
